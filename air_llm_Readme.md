...
Adds `airllm_kaggle_pipeline.py` - a complete pipeline to run **Qwen2.5-72B-Instruct** (or any 70B+ model) on Kaggle's free T4 GPU using AirLLM's layer-by-layer inference.

## What's New

| File | Purpose |
|------|---------|
| `airllm_kaggle_pipeline.py` | Full pipeline: download, prune HTML, run 3-task prompt via AirLLM, enforce token budget |
| `requirements_airllm.txt` | Dependencies for AirLLM pipeline |
| `README.md` | Updated with AirLLM docs, model recommendations, Kaggle setup |

## Key Features

- **Layer-by-layer inference**: Only 1 transformer layer in VRAM at a time (72B model in 16GB)
- **4-bit compression**: Further reduces memory footprint
- **Checkpoint/resume**: Interrupted Kaggle sessions resume automatically
- **Response validation**: Verifies all 3 tasks (Cleaner, Indexer, Router) are present
- **Enhanced HTML pruning**: Removes empty tags, strips `<head>` entirely, cleans deeper
- **Auto GPU detection**: Falls back to CPU if no GPU available

## Model Recommendation

**Qwen/Qwen2.5-72B-Instruct** is the best choice for this pipeline because:
1. Superior verbatim HTML-to-Markdown translation
2. Excellent table and structured output preservation
3. Strong instruction following for the 3-task format
4. Native support for ChatML prompt template
